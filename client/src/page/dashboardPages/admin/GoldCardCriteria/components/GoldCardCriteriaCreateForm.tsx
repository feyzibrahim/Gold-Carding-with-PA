...
function GoldCardCriteriaCreateForm({ setShowModal, setData }: Props) {
  const { user } = useAppSelector((state) => state.user);
  const handleCreateCriteria = async (values: any) => {
    try {
      ...
      if (res.success) {
        setData((prevData: any[]) => [res.data, ...prevData]);
      }
      setShowModal(false);
    } catch (error) {
...
export default GoldCardCriteriaCreateForm;
